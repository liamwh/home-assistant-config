...
class StartSpotify(hass.Hass):
    def initialize(self):
        self.input_boolean = self.args.get(
            "input_boolean", DEFAULT_INPUT_BOOLEAN)
        self.call_spotify = partial(
            self.call_service, entity_id="media_player.spotify")
    ...
    def start(self, **kwargs):
        ...
        app = self.get_app("start_tv_spotfy")
        self.listen_event(self.select_source, app.done_signal,
                          timeout=30, oneshot=True)
        app.start(**kwargs)
    ...
    def select_source(self, event=None, data=None, kwargs=None):
        ...
        if not self.source_available(spotify_source):
            ...
            self.call_spotify("media_player/select_source",
